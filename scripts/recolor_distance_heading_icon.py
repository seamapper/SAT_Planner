"""Create an orange distance/heading icon that is pixel-identical to the cyan
one except for cyan/teal accents remapped to orange.
Navy background and non-accent pixels are left unchanged.
"""
...
SRC = ASSETS / "distance_heading_map_icon.png"
OUT_ORANGE = ASSETS / "distance_heading_map_icon_active.png"
PROJECT_MEDIA = Path(
...
def is_accent_pixel(r: int, g: int, b: int, a: int) -> bool:
    """True for cyan/teal accent pixels; False for navy bg / gray shadows."""
    if a < 8:
        return False
...
def main() -> None:
    ...
    if PROJECT_MEDIA.exists():
        (PROJECT_MEDIA / "distance_heading_map_icon.png").write_bytes(SRC.read_bytes())
        recolor(SRC, PROJECT_MEDIA / "distance_heading_map_icon_active.png")
